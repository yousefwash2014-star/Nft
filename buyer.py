...
REASON_MESSAGES_AR = {
    ...
    "listing_gas_too_high": "رسوم الغاز للإدراج مرتفعة",
    "listing_failed": "فشل إدراج NFT",
    "listing_approved": "تم الموافقة على الإدراج",
    "listed_successfully": "تم الإدراج بنجاح",
}
...
def calculate_listing_price(
    ...
    """
    حساب سعر الإدراج بناءً على الوضع المحدد.
    
    modes:
    - "floor": سعر الأرضية + نسبة مئوية
    - "mint": سعر المينت + نسبة مئوية
    - "fixed": سعر ثابت (يتم تحديده من offset_percent كنسبة من المينت)
    """
    if mode == "floor" and floor_price is not None and floor_price > 0:
        base_price = floor_price
    elif mode == "mint":
        base_price = mint_price_eth
    else:
        base_price = mint_price_eth
    
    multiplier = 1 + (offset_percent / 100)
    final_price = base_price * multiplier
    
    return max(0.001, final_price)
