...
def apply_poa_middleware(w3: Web3, chain_name: str) -> None:
    """
...
def get_wallet_balance_usd(w3: Web3, wallet_address: str, chain_name: str, eth_price_usd: float) -> float:
    """
    يرجع رصيد المحفظة بالدولار - لكل سلسلة بشكل منفصل مع معالجة أفضل للأخطاء.
    """
    now = time.time()
    ...
    try:
        ...
        log.error(f"❌ [TO_CHECKSUM] {chain_name} - خطأ في تحويل العنوان: {e}")
        return 0.0
...
def get_all_wallets_balances(w3_instances: dict, wallets: list, eth_price_usd: float) -> dict:
    """
    جلب رصيد جميع المحافظ على جميع السلاسل - مع تجنب Rate Limiting.
    """
    balances = {}
    ...
    for idx, wallet in enumerate(wallets):
        ...
        for chain_name, w3 in w3_instances.items():
            try:
                ...
                log.info(f"💰 {chain_name} - {wallet_name}: ${balance:.4f}")
            except Exception as e:
