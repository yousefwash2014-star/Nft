...
# ============================================================================
# استيراد نظام الإدراج التلقائي
# ============================================================================
from auto_lister import (
    init_auto_lister, list_nft_after_mint, get_auto_lister_status,
    set_auto_lister_config,
)
from listing_manager import get_listing_manager
...
# ============================================================================
# إعدادات الإدراج التلقائي
# ============================================================================
AUTO_LIST_ENABLED = os.environ.get("AUTO_LIST_ENABLED", "false").strip().lower() == "true"
MAX_LISTING_GAS_USD = float(os.environ.get("MAX_LISTING_GAS_USD", "0.01"))
LISTING_PRICE_MODE = os.environ.get("LISTING_PRICE_MODE", "floor").strip()
LISTING_PRICE_OFFSET_PERCENT = float(os.environ.get("LISTING_PRICE_OFFSET_PERCENT", "10.0"))
LISTING_DURATION_DAYS = int(os.environ.get("LISTING_DURATION_DAYS", "30"))

# تحديث إعدادات auto_lister
set_auto_lister_config(
    enabled=AUTO_LIST_ENABLED,
    max_gas_usd=MAX_LISTING_GAS_USD,
...
SENT: Dict[str, Set[str]] = {
    k: set() for k in ["discovery", "result", "status", "retry", "soldout", "permanent", "progress"]
}
...
def load_wallets():
    ...
    for name, pk, addr in cfgs:
        pkv = os.environ.get(pk)
        addrv = os.environ.get(addr)
        if pkv and addrv:
            wallets.append({
                "name": name,
                "private_key": pkv.strip(),
                "address": addrv.strip()
            })
    return wallets
...
# ============================================================================
# متغيرات الإدراج التلقائي
# ============================================================================
_auto_lister_instance = None
listing_manager = get_listing_manager()
...
class SmartScanner:
    ...
    async def process(self, session):
        ...
        await asyncio.gather(
            *[handle_mint(session, s, "ethereum") for s in batch],
...
async def process_wallet(session, slug, detail, stage, cn, wallet, ep, end_time=None):
    ...
    if result:
        ...
        result.collection_slug = slug
        result.contract_address = contract
    
    if result and not result.success:
        send_once(
            "status",
            f"{slug}_{wn}_{result.reason}",
            build_status_msg(wn, detail, result.reason)
        )
    ...
    if result and result.success and result.tx_hash:
        if await confirm_transaction(w3, result.tx_hash):
            ...
            # ================================================================
            # الإدراج التلقائي بعد نجاح الشراء
            # ================================================================
            if AUTO_LIST_ENABLED and OPENSEA_API_KEY:
                # محاكاة الحصول على token_id - في الواقع يجب استخراجه من حدث Mint
                # هذا تنفيذ مبسط، في الإنتاج يجب استخراج token_id من المعاملة
                token_id = str(int(time.time()) % 10000 + 1)
                
                listing_result = await list_nft_after_mint(
                    mint_result=result,
                ...
                if listing_result and listing_result.success:
                    send_telegram(
                        f"✅ تم إدراج NFT تلقائياً\n\n"
                        f"المجموعة: {slug}\n"
                        f"المحفظة: {wn}\n"
                        f"السعر: {listing_result.price_eth:.4f} ETH\n"
                        f"السلسلة: {cn}"
                    )
        else:
...
async def retry_loop(session, tracker):
    ...
    while True:
        ...
        if tracker.attempt_count % RETRY_NOTIFY_EVERY == 0:
            send_once(
                "progress",
                f"{key}_{tracker.attempt_count}",
                build_retry_progress_msg(tracker.wallet_name, name, tracker.attempt_count)
            )
        ...
        if result and is_reason_permanent(result.reason):
            ...
            send_once(
                "permanent",
                key,
                build_permanent_msg(tracker.wallet_name, result.reason_text)
            )
            return
...
async def run():
    ...
    # ================================================================
    if AUTO_LIST_ENABLED and OPENSEA_API_KEY:
        _auto_lister_instance = await init_auto_lister(
            api_key=OPENSEA_API_KEY,
            wallets=WALLETS,
            enabled_chains=ENABLED_CHAINS,
            w3_instances=w3_instances
        )
        if _auto_lister_instance:
            status = await get_auto_lister_status()
            send_telegram(
                f"🟢 تم تفعيل نظام الإدراج التلقائي\n\n"
                f"وضع التسعير: {LISTING_PRICE_MODE}\n"
                f"نسبة الربح: {LISTING_PRICE_OFFSET_PERCENT}%\n"
                f"حد رسوم الغاز: ${MAX_LISTING_GAS_USD}\n"
                f"مدة الإدراج: {LISTING_DURATION_DAYS} يوم"
            )
    ...
    send_telegram(
        ...
        f"الإدراج التلقائي: {'مفعل' if AUTO_LIST_ENABLED else 'معطل'}\n"
        f"النظام جاهز للعمل"
    )
